...
public class UsuariorolController {
	...
	public ResponseEntity<List<Usuariorol>> readAll() {
		try {
			List<Usuariorol> usuarioroles = service.readAll();
	...
	@PostMapping
	public ResponseEntity<Usuariorol> crearUsuariorol(@Valid @RequestBody Usuariorol ur) {
		try {
			Usuariorol us = service.create(ur);
	...
	public ResponseEntity<Usuariorol> getUsuariorolId(@PathVariable("id") Long id) {
		try {
			Usuariorol us = service.read(id).get();
			return new ResponseEntity<>(us, HttpStatus.CREATED);
		} catch (Exception e) {
			// TODO: handle exception
			return new ResponseEntity<>(null, HttpStatus.NO_CONTENT);
	...
	@PutMapping("/{id}")
	public ResponseEntity<Usuariorol> updateUsuariorol(@PathVariable("id") Long id, @Valid @RequestBody Usuariorol ur) {
		Optional<Usuariorol> us = service.read(id);
		if (us.isEmpty()) {
			return new ResponseEntity<>(HttpStatus.NO_CONTENT);
		} else {
			return new ResponseEntity<>(service.update(ur), HttpStatus.OK);
